	...
	<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
		{#each algorithms as algo (algo.id)}
			<a
				...
				class="bg-surface-50 border-surface-200 hover:border-primary group flex flex-col rounded-xl border p-6 shadow-sm transition-all hover:shadow-md"
			>
				<div class="flex items-start justify-between">
					<h2 class="group-hover:text-primary text-xl font-bold transition-colors">
						{algo.name}
					</h2>
					<span class="bg-surface-200 rounded-full px-2 py-1 text-xs font-mono"
						>{algo.complexity.average}</span
					>
				</div>
